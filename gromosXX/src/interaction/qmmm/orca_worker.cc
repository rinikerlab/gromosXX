...
int interaction::Orca_Worker::write_input_coordinates(std::ofstream& ifs
  ...
  ifs << qm_zone.qm.size() + qm_zone.link.size() << '\n' << "Coordinates of the QM zone (in Ångström) generated by GROMOS" << '\n';
...
void interaction::Orca_Worker::write_coordinate_header(std::ofstream& ifs
                                                     , const QM_Zone& qm_zone) const {
  // XYZ format
  ifs << qm_zone.qm.size() + qm_zone.link.size()  << '\n'
      << "Automatically generated with GROMOS"    << '\n';
}
