...
static int
_calculate_dfunct_substitution_form(topology::Topology &topo,
  ...
  DEBUG(10,
        "DFUNCT atom_i " << atom_i << math::v2s(conf.current().pos(atom_i)));
  DEBUG(10,
        "DFUNCT atom_j " << atom_j << math::v2s(conf.current().pos(atom_j)));
  DEBUG(10,
        "DFUNCT atom_k " << atom_k << math::v2s(conf.current().pos(atom_k)));
  DEBUG(10,
        "DFUNCT atom_l " << atom_l << math::v2s(conf.current().pos(atom_l)));
  math::Vec dist_vec_ji, dist_vec_lk;
  math::Periodicity<B> periodicity(conf.current().box);
  periodicity.nearest_image(conf.current().pos(atom_j),
                            conf.current().pos(atom_i), dist_vec_ji);
  periodicity.nearest_image(conf.current().pos(atom_l),
                            conf.current().pos(atom_k), dist_vec_lk);
  ...
  double V_bias = 0.5 * force * (dist_ji + d * dist_lk - r_0) *
                  (dist_ji + d * dist_lk - r_0);
  ...
  conf.current()
      .energies.distanceres_energy[topo.atom_energy_group()[atom_i]] +=
      0.25 * V_bias;
  conf.current()
      .energies.distanceres_energy[topo.atom_energy_group()[atom_j]] +=
      0.25 * V_bias;
  conf.current()
      .energies.distanceres_energy[topo.atom_energy_group()[atom_k]] +=
      0.25 * V_bias;
  conf.current()
      .energies.distanceres_energy[topo.atom_energy_group()[atom_l]] +=
      0.25 * V_bias;
  return 0;
}
...
template <math::boundary_enum B>
static int
_calculate_dfunct_cycloaddition_form(topology::Topology &topo,
                                     configuration::Configuration &conf,
                                     simulation::Simulation &sim) {
  // shorten the code
  int atom_i = sim.param().dfunct.atom_i;
  ...
  periodicity.nearest_image(conf.current().pos(atom_j),
                            -1.0 * conf.current().pos(atom_i), dist_vec_ji);
  periodicity.nearest_image(conf.current().pos(atom_l),
                            -1.0 * conf.current().pos(atom_k), dist_vec_lk);
  double dist_ji = math::abs(dist_vec_ji);
  double dist_lk = math::abs(dist_vec_lk);
  DEBUG(30, "DFUNCT dist_vec_ji " << math::v2s(dist_vec_ji));
  DEBUG(30, "DFUNCT dist_vec_lk " << math::v2s(dist_vec_lk));
  ...
  periodicity.nearest_image(dist_vec_lk_halfs, dist_vec_ji_halfs,
                            dist_vec_lkji);
...
int interaction::DFunct_Interaction::calculate_interactions(
    topology::Topology &topo, configuration::Configuration &conf,
    simulation::Simulation &sim) {
  // atomic distances expressed as vectors
  // in GROMOS vector r_ji is defined as the vector from point i to point j (r_j
  // - r_i) find nearest periodic copies
  switch (sim.param().dfunct.dfunct) {
  case simulation::dfunct_substitution:
    // restrain the distances of an incoming (i) and outgoing (l) atom to a
    // central atom (j,k), respectively
    SPLIT_BOUNDARY(_calculate_dfunct_substitution_form, topo, conf, sim);
    break;

  case simulation::dfunct_cycloaddition:
    // restrain the distance between two cycloaddition reaction partners (i, j)
    // and (k, l), respectively
    SPLIT_BOUNDARY(_calculate_dfunct_cycloaddition_form, topo, conf, sim);
    break;

  default:
    io::messages.add("DFUNCT functional not implemented", "DFunct_Interaction",
                     io::message::critical);
    break;
  }
